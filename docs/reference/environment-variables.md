...
| `RETRIEVAL_INDEX_DIR` | `retrieval.persistence.cache_base_dir` | `~/.cache/agentic-retrieval/indexes` | Relocates the on-disk index cache base directory — see [persistence and cache](persistence-and-cache.md) for the self-indexing feedback-loop warning before pointing it inside a project root |
